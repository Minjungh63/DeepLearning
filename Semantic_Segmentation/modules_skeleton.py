...
def train_model(trainloader, model, criterion, optimizer,scheduler, device):
    ...
    for i, (inputs, labels) in enumerate(trainloader):
        ...
        inputs = inputs.to(device)                              # "images" = "inputs"
        labels = labels.to(device=device, dtype=torch.int64)    # "labels" = "targets"
...
def get_loss_train(model, trainloader, criterion, device):
    ...
    for batch, (inputs, labels) in enumerate(trainloader):
        with torch.no_grad():
            ...
            #########################################
            # difference: don't backpropagate loss in test
            outputs = model(inputs)         # (Image 수, Channel 수, Height , Width)
            loss = criterion(outputs, labels)

            outputs = np.transpose(outputs.cpu(), (0,2,3,1))    # (Image 수, Height , Width, Channel 수)
            preds = torch.argmax(outputs, dim=3).float()        # 3번째 축(Channel) 기준 argmax. preds[Image idx][Height idx][Width idx]
...
def val_model(model, valloader, criterion, device, dir):
    ...
    for batch, (inputs, labels) in enumerate(valloader):
        with torch.no_grad():
            ...
            for i in range(preds.shape[0]): # preds.shape[0]: image idx
                temp = preds[i].cpu().data.numpy()  # (Height , Width), value: 해당 위치(H,W)에서 가장 큰 value를 가지는 channel의 번호
                temp_l = labels[i].cpu().data.numpy()   #  (Height , Width), value: 해당 이미지의 label 번호
                temp_rgb = np.zeros((temp.shape[0], temp.shape[1], 3))  # temp.shape[0]: Height, temp.shape[1]: Width
                ...
                for j in range(temp.shape[0]):
                    for k in range(temp.shape[1]):
                        ...
                        #########################################
                        temp_rgb[j][k][0] = cls_invert[temp_l[j][k]][0]
                        temp_rgb[j][k][1] = cls_invert[temp_l[j][k]][1]
                        temp_rgb[j][k][2] = cls_invert[temp_l[j][k]][2]
                        temp_label[j][k][0] = cls_invert[temp[j][k]][0]
                        temp_label[j][k][1] = cls_invert[temp[j][k]][1]
                        temp_label[j][k][2] = cls_invert[temp[j][k]][2]

                img = inputs[i].cpu()
                img = np.transpose(img, (2, 1, 0))  # (Channel 수, Width, Height)
