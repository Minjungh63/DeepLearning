    ...
    def __init__(self, in_channels, middle_channels, out_channels, downsample=False):
        ...
        if self.downsample:
            self.layer = nn.Sequential(
                ...
                # number of channels: in_channels -> middle_channels, size: be halved
                conv3x3(middle_channels, middle_channels, 1, 1),  # number of channels & size: no changed
                conv1x1(middle_channels, out_channels, 1, 0)
                # number of channels: middle_channels -> out_channels, size: no changed
            )
...
class UNetWithResnet50Encoder(nn.Module):
    ...
    def forward(self, x, with_output_feature_map=False): #256
        ...
        x = self.UpConv1(x)
        x = torch.cat([out2, x], dim=1)#######fill in here ####### hint : concatenation (Practice Lecture slides 6p)
        x = self.UnetConv1(x)
        x = self.upconv2_1(x, output_size=torch.Size([x.size(0), 256, 64, 64]))
        x = self.upconv2_2(x)
        x = torch.cat([out1, x], dim=1) #######fill in here ####### hint : concatenation (Practice Lecture slides 6p)
