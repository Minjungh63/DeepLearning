...
Mat meanfilter(const Mat input, int n, const char* opt) {
	...
	int row = input.rows;
	int col = input.cols;
	int kernel_size = (2 * n + 1); //-n ~ n: 2*n+1
	int tempa;
	int tempb;
 // Initialiazing Kernel Matrix 
	kernel = Mat::ones(kernel_size, kernel_size, CV_32F) / (float)(kernel_size * kernel_size); // Mat::ones(...): returns a matrix with all elements as one
	float kernelvalue=kernel.at<float>(0, 0);  // To simplify, as the filter is uniform. All elements of the kernel value are same.
	
	Mat output = Mat::zeros(row, col, input.type()); // Mat:zeros(...): returns a matrix with all elements as zero
	
	
	for (int i = 0; i < row; i++) { //for each pixel in the output
		for (int j = 0; j < col; j++) {
			
			if (!strcmp(opt, "zero-paddle")) { //1. type of boundary processing: zero-paddle
				float sum1 = 0.0;
			...
			else if (!strcmp(opt, "mirroring")) { //2. type of boundary processing: mirroring
				float sum1 = 0.0;
				for (int a = -n; a <= n; a++) { // for each kernel window
					for (int b = -n; b <= n; b++) {
			...
			else if (!strcmp(opt, "adjustkernel")) { //3. type of boundary processing: adjustkernel
				float sum1 = 0.0;
				float sum2 = 0.0;
				for (int a = -n; a <= n; a++) { // for each kernel window
					for (int b = -n; b <= n; b++) {
