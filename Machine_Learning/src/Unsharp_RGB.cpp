...
Mat UnsharpMask(const Mat input, int n, float sigmaT, float sigmaS, const char* opt, float k) {
		...
		for (int j = 0; j < col; j++) {
			...
			if (!strcmp(opt, "zero-paddle")) {
				...
				for (int a = -n; a <= n; a++) {
					for (int b = -n; b <= n; b++) {
						if ((i + a <= row - 1) && (i + a >= 0) && (j + b <= col - 1) && (j + b >= 0)) {
							sum1_r += (kernel.at<float>(a + n, b + n) / sum2) * (float)(input.at<C>(i + a, j + b)[0]);
							sum1_g += (kernel.at<float>(a + n, b + n) / sum2) * (float)(input.at<C>(i + a, j + b)[1]);
							sum1_b += (kernel.at<float>(a + n, b + n) / sum2) * (float)(input.at<C>(i + a, j + b)[2]);
						}
					}
				}
				output.at<C>(i, j)[0] = (G)sum1_r;
	...
	for (int i = 0; i < row; i++) {
		for (int j = 0; j < col; j++) {
			output.at<C>(i, j)[0] = (input.at<C>(i, j)[0] - k * output.at<C>(i, j)[0]) / (1 - k); // (I-kL)/(1-k)
			output.at<C>(i, j)[1] = (input.at<C>(i, j)[1] - k * output.at<C>(i, j)[1]) / (1 - k);
			output.at<C>(i, j)[2] = (input.at<C>(i, j)[2] - k * output.at<C>(i, j)[2]) / (1 - k);
		}
	}
	return output;
}